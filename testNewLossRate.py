...
for i in range(15000):
    sess.run(train, feed_dict={xs: input1, ys: output12})

result = 0

predictionResult = sess.run(prediction, feed_dict={xs: input2, ys: output22})

sumError = 0

for i in range(200):

    error = float(predictionResult[i]*10) - float(output22[i]*10)

    sumError = sumError + abs(error)

print((sumError/200)/float(output22[i]*10))
